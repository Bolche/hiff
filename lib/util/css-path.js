...
function cssPath($node) {
  ...
  if (!$node.length || (node_type != 'element' && node_type != 'root'))
    return undefined;
  ...
  if (parent.length) {
    ...
    if (typeof parentPath === 'undefined') return nodeSelector($node);
    else return parentPath + ' > ' + nodeSelector($node);
  } else {
...
function nodeSelector($node) {
  ...
  if (matchesAnythingElse) {
    ...
    return specifier + ':nth-of-type('+index+')';
  } else {
    // unique enough!
    return specifier;
  }
