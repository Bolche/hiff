...
function added($addedNode, $parentBefore, indexBefore, $parentAfter, indexAfter) {
  return {
    ...
    message: "Added:    " + stringify($addedNode),
  };
}
...
function removed($removedNode, $parentBefore, indexBefore, $parentAfter, indexAfter) {
  return {
    ...
    message: "Removed:  " + stringify($removedNode)
  };
}
...
function changed($nodeBefore, $nodeAfter) {
  ...
  var diffs = diff.diffChars(
    stringify($nodeBefore).trim().replace(/\s+/g, ' '), 
    stringify($nodeAfter).trim().replace(/\s+/g, ' ')
  );
  ...
  return {
    ...
    changes: diffs,
    message: "Modified: " + changes(diffs)
  };
}
